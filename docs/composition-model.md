...
- `profiles/` describe product goals such as `saas-web-app`, `mcp-service`, `desktop-app`, and `browser-extension`.
- `runtimes/` define the core language/runtime skeleton such as `python`, `node`, or `rust`.
- `modules/` add technical capabilities such as `fastapi`, `htmx`, `postgres`, `alembic`, `playwright-pdf`, `tauri`, or `chrome-mv3`.
